...
rDownloadURL = {"main": "https://assets.cdn.pandawa.live/xcui/xcui/main.tar.gz", "sub": "https://assets.cdn.pandawa.live/xcui/xcui/sub.tar.gz"}
rPackages = ["libgeoip-dev", "libxslt1-dev", "e2fsprogs", "wget", "mcrypt", "nscd", "htop", "mariadb-server"]
...
def prepare(rType="MAIN"):
    ...
    os.system("wget -q -O /tmp/libpng12.deb http://ppa.launchpad.net/linuxuprising/libpng12/ubuntu/pool/main/libp/libpng/libpng12-0_1.2.54-1ubuntu1.1+1~ppa0~focal_amd64.deb")
    os.system("dpkg -i /tmp/libpng12.deb > /dev/null")
    printc("Installing libssl")
    os.system("wget -q -O /tmp/libssl.deb http://archive.ubuntu.com/ubuntu/pool/main/o/openssl1.0/libssl1.0.0_1.0.2n-1ubuntu5.3_amd64.deb")    # download deb file
    os.system("dpkg -i /tmp/libssl.deb")    # install deb package
    os.system("wget -q -O /tmp/libonig.deb http://archive.ubuntu.com/ubuntu/pool/universe/libo/libonig/libonig4_6.7.0-1_amd64.deb")
    os.system("dpkg -i /tmp/libonig.deb")
    os.system("wget -q -O /tmp/paramiko.deb http://security.ubuntu.com/ubuntu/pool/main/p/paramiko/python-paramiko_2.0.0-1ubuntu1.2_all.deb")
    os.system("dpkg -i /tmp/paramiko.deb")
    os.system("apt --fix-broken install > /dev/null")
    os.system("apt-get install -y > /dev/null") # Clean up above
    try: os.remove("/tmp/libpng12.deb")
    except: pass
    try: os.remove("/tmp/libssl.deb")
    except: pass
    try: os.remove("/tmp/libonig.deb")
    except: pass
    try: os.remove("/tmp/paramiko.deb")
    except: pass
    try:
...
def modifyNginx():
    ...
    if not "listen 8080;" in rPrevData:
...
if __name__ == "__main__":
    printc("Xtream Codes Reborn Focal - Install", col.OKGREEN, 2)
    ...
    if rType.upper() in ["MAIN", "LB"]:
        ...
        if len(rHost) > 0 and len(rPassword) > 0 and rServerID > -1:
            ...
            if raw_input("  ").upper() == "Y":
                ...
                printc("Admin UI: https://%s:2096" % getIP())
                if rType.upper() == "MAIN":
